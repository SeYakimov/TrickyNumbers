...
class ChooseDifficult extends State implements InputProcessor {
    ...
    ChooseDifficult(GameStateManager gsm, AssetManager manager, IActivityRequestHandler handler) {
        ...
        myColor = com.east71.trickynumbers.models.RColor.getColor(pref.getString("COLOR"));
        ...
        btnEASY = new com.east71.trickynumbers.models.TextButton("EASY", myColor, PADDING,
                PADDING + 2 * (GAP + buttonHeight),
                w - PADDING * 2 - GAP - buttonWidth, buttonHeight, font);
        btnMEDIUM = new com.east71.trickynumbers.models.TextButton("NORMAL", myColor, PADDING, PADDING + GAP + buttonHeight,
                w - PADDING * 2 - GAP - buttonWidth, buttonHeight, font);
        btnHARD = new com.east71.trickynumbers.models.TextButton("HARD", myColor, PADDING, PADDING,
                w - PADDING * 2 - GAP - buttonWidth, buttonHeight, font);

        btnEASYHigh = new com.east71.trickynumbers.models.TextButton("" + pref.getInteger("HIGHSCORE_EASY", 0), myColor, highScoresX,
                PADDING + 2 * (GAP + buttonHeight),
                buttonWidth, buttonHeight, font);
        btnMEDIUMHigh = new com.east71.trickynumbers.models.TextButton("" + pref.getInteger("HIGHSCORE_MEDIUM", 0), myColor, highScoresX, PADDING + GAP + buttonHeight,
                buttonWidth, buttonHeight, font);
        btnHARDHigh = new com.east71.trickynumbers.models.TextButton("" + pref.getInteger("HIGHSCORE_HARD", 0), myColor, highScoresX, PADDING,
                buttonWidth, buttonHeight, font);
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button) {
        ...
        if(pointer < touches.size()) {
            ...
            if (touches.get(pointer).button != null && touches.get(pointer).button.equals(btnEASY)){
                ...
                if (btnEASY.contains(v)) {
                    gsm.push(new com.east71.trickynumbers.states.PlayState(gsm, manager, handler, com.east71.trickynumbers.states.Difficulty.EASY));
                }
            }
            if (touches.get(pointer).button != null && touches.get(pointer).button.equals(btnMEDIUM)){
                btnMEDIUM.setPressed(false);
                if (btnMEDIUM.contains(v)){
                    gsm.push(new com.east71.trickynumbers.states.PlayState(gsm, manager, handler, com.east71.trickynumbers.states.Difficulty.MEDIUM));
                }
            }
            if (touches.get(pointer).button != null && touches.get(pointer).button.equals(btnHARD)){
                btnHARD.setPressed(false);
                if (btnHARD.contains(v)){
                    gsm.push(new com.east71.trickynumbers.states.PlayState(gsm, manager, handler, com.east71.trickynumbers.states.Difficulty.HARD));
                }
            }
